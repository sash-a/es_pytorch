...
if __name__ == '__main__':
    ...
    for gen in range(cfg.general.gens):  # main loop
        ...
        max_ind = np.ndarray.argmax(np.concatenate((pos_fits, neg_fits)))
        max_fit = pos_fits[max_ind % len(pos_fits)]
        coeff = 1
        if max_ind > len(pos_fits):
            max_fit = neg_fits[max_ind % len(pos_fits)]
            coeff = -1

        if max_fit > best_fit:
            best_fit = max_fit
            max_noise = coeff * nt[int(inds[max_ind % len(pos_fits)])]
            policy.flat_params += max_noise

        if comm.rank == 0:
            print(f' best fitness:{best_fit}')
            print(f' max fitness:{max_fit}')
            print(f' avg fitness:{np.mean(np.concatenate((pos_fits, neg_fits)))}')
            print(f' min fitness:{np.min(np.concatenate((pos_fits, neg_fits)))}\n\n')
